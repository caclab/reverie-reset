...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    --itemsTable = sents
    for k=1,#sents do
      newImgIndex = tostring(tonumber(opt.image_index) - 1)
      --print("NEW IMAGE INDEX:", tostring(tonumber(entry.image_id + newImgIndex)))
      newEntryData = tostring(tonumber(data.infos[k].id) + tonumber(newImgIndex))
      local entry = {image_id = newEntryData, caption = sents[k]}
      ...
      if opt.dump_images == 1 then
        ...
        local cmd = 'cp "' .. path.join(opt.image_root, data.infos[k].file_path) .. '" /home/fitosegrera/technoxaman/cac/a-and-t/yanlei/code/reverie-reset/server/public/imgs/captioned/' .. newEntryData .. '.jpg' -- bit gross
        --print(cmd)
      ...
      if verbose then
        print(string.format('image %s: %s', tostring(tonumber(entry.image_id) + newImgIndex), entry.caption))
      end
      itemsCounter = itemsCounter + 1
      -- local d = {image_id = entry.image_id, caption = entry.caption}
      -- itemsTable.insert(d)
    end
...
if opt.dump_json == 1 then
  local file = io.open("/home/fitosegrera/technoxaman/cac/a-and-t/yanlei/code/reverie-reset/server/public/imgs/captioned/vis.json", "r")
  local t1
  if file then
    t1 = json.decode(file:read("*all"))
    --print(t1)
    file:close()
  end
  for k,v in ipairs(split_predictions) do
    table.insert(t1, v)
  end

  print("==============")
  print(t1)
  print(type(split_predictions))
  -- for i=1,itemsCounter do
  --   print(string.format('image %s: %s', tostring(tonumber(entry.image_id) + newImgIndex), entry.caption))
  -- end
  -- dump the json
  --print(split_predictions)
  utils.write_json('/home/fitosegrera/technoxaman/cac/a-and-t/yanlei/code/reverie-reset/server/public/imgs/captioned/vis.json', t1)
end
