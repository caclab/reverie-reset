...
local jsonData = jsonFile("/home/fitosegrera/technoxaman/cac/a-and-t/yanlei/code/reverie-reset/server/public/imgs/captioned/vis.json")
...
-- Basic options
cmd:option('-image_index', 1, 'starting index for image renaming')
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    for k=1,#sents do
      newImgIndex = tostring(k + tonumber(opt.image_index) - 1)
      print('NEW FILE INDEX: ', newImgIndex, sent, k)
      opt.num_images = newImgIndex
      ...
      if opt.dump_images == 1 then
        ...
        local cmd = 'cp "' .. path.join(opt.image_root, data.infos[k].file_path) .. '" /home/fitosegrera/technoxaman/cac/a-and-t/yanlei/code/reverie-reset/server/public/imgs/captioned/' .. newImgIndex .. '.jpg' -- bit gross
        --print(cmd)
        os.execute(cmd) -- dont think there is cleaner way in Lua
        --delete the image
        local cmd2 = 'rm "' .. path.join(opt.image_root, data.infos[k].file_path) .. '"'
        --print('deleting...')
        os.execute(cmd2) -- dont think there is cleaner way in Lua
      end
      if verbose then
...
split_predictions[1]["image_id"] = opt.num_images
--print('TO PRINT:', split_predictions[1]["image_id"])
jsonTable[split_predictions[1]["image_id"]] = split_predictions[1]["caption"]
--print(jsonTable)
--print('loss: ', loss)
if lang_stats then
  print(lang_stats)
...
if opt.dump_json == 1 then
  ...
  -- utils.write_json('/home/fitosegrera/technoxaman/cac/a-and-t/yanlei/code/reverie-reset/server/public/imgs/captioned/vis.json', jsonTable)
  utils.write_json('/home/fitosegrera/technoxaman/cac/a-and-t/yanlei/code/reverie-reset/server/public/imgs/captioned/vis.json', split_predictions)
  -- local file, err = io.open ('/home/fitosegrera/technoxaman/cac/a-and-t/yanlei/code/reverie-reset/server/public/imgs/captioned/vis.txt',"w")
  -- if file==nil then
